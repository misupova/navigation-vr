...
public class Line : MonoBehaviour
{
	...
	public void Initialise(Vector2 start, Vector2 end, float height, float width, Color color)
	{
		startPosition = start;
		endPosition = end;
		lineHeight = height;
		...
		lineRenderer.transform.Rotate(90.0f, 0.0f, 0.0f, Space.World);

		Vector3 setHeight = new Vector3(0, height, 0);
		lineRenderer.transform.position += setHeight; 
	}
	...
	private void UpdatePosition ()
	{
		...
		Vector3 centerPos = new Vector3 (startPosition.x + endPosition.x, 0f, startPosition.y + endPosition.y) / 2;
